...
from .simple_hyper_nets_no_head_last_layer import HyperNetwork2Layer, HyperNetworkEmbedInput2Layer
...
class HyperAC2Layer(nn.Module):
    ...
    def __init__(self, meta_v_dim, state_dim, action_dim, hyper_hidden_dim=128, dynamic_hidden_dim=128):
        ...
        self.actor_base = nn.Sequential(nn.Linear(state_dim + meta_v_dim, dynamic_hidden_dim),
                                        nn.ReLU(),
                                        nn.Linear(dynamic_hidden_dim, dynamic_hidden_dim),
                                        nn.ReLU()
                                        )
        self.actor_base.apply(init_weights_kaiming_uniform)
        self.actor_hyper = HyperNetwork2Layer(meta_v_dim, action_dim, hyper_hidden_dim, dynamic_hidden_dim)
        self.critic_base = nn.Sequential(nn.Linear(state_dim + meta_v_dim, dynamic_hidden_dim),
                                        nn.ReLU(),
                                        nn.Linear(dynamic_hidden_dim, dynamic_hidden_dim),
                                        nn.ReLU()
                                        )
        self.critic_base.apply(init_weights_kaiming_uniform)
        self.critic_hyper = HyperNetwork2Layer(meta_v_dim, 1, hyper_hidden_dim, dynamic_hidden_dim)
    ...
    def act(self, meta_v, state, ret_prob=False, pos_emb=None):
        actor_w, actor_b = self.actor_hyper(meta_v)
        actor_base_out = self.actor_base(torch.cat([state, meta_v], dim=1))
        action_probs = torch.bmm(actor_w, actor_base_out.unsqueeze(2)) + actor_b
        action_probs = torch.squeeze(action_probs, dim=2)
    ...
    def evaluate(self, meta_v, state, action, pos_emb=None):
        ...
        critic_w, critic_b = self.critic_hyper(meta_v)
        critic_base_out = self.critic_base(torch.cat([state, meta_v], dim=1))
        state_values = torch.bmm(critic_w, critic_base_out.unsqueeze(2)) + critic_b
        state_values = torch.squeeze(state_values, dim=2)
...
class HyperACEmbedInput2Layer(nn.Module):
    ...
    def __init__(self, meta_v_dim, meta_v_emb_dim, state_dim, action_dim, hyper_hidden_dim=128, dynamic_hidden_dim=128, scale=True):
        ...
        self.scale = scale
        self.actor_base = nn.Sequential(nn.Linear(state_dim + meta_v_emb_dim, dynamic_hidden_dim),
                                        nn.ReLU(),
                                        nn.Linear(dynamic_hidden_dim, dynamic_hidden_dim),
                                        nn.ReLU()
                                        )
        self.actor_base.apply(init_weights_kaiming_uniform)
        self.actor_hyper = HyperNetworkEmbedInput2Layer(meta_v_dim, meta_v_emb_dim, action_dim, hyper_hidden_dim, dynamic_hidden_dim)
        self.critic_base = nn.Sequential(nn.Linear(state_dim + meta_v_emb_dim, dynamic_hidden_dim),
                                         nn.ReLU(),
                                         nn.Linear(dynamic_hidden_dim, dynamic_hidden_dim),
                                         nn.ReLU()
                                         )
        self.critic_base.apply(init_weights_kaiming_uniform)
        self.critic_hyper = HyperNetworkEmbedInput2Layer(meta_v_dim, meta_v_emb_dim, 1, hyper_hidden_dim, dynamic_hidden_dim)
    ...
    def act(self, meta_v, state, ret_prob=False, pos_emb=None):
        actor_w, actor_b, actor_emb = self.actor_hyper(meta_v, pos_emb=pos_emb, scale=self.scale)
        actor_base_out = self.actor_base(torch.cat([state, actor_emb], dim=1))
        action_probs = torch.bmm(actor_w, actor_base_out.unsqueeze(2)) + actor_b
        action_probs = torch.squeeze(action_probs, dim=2)
        dist = Categorical(logits=action_probs)
        action = dist.sample()
    ...
    def evaluate(self, meta_v, state, action, pos_emb=None):
        ...
        action_logprobs = dist.log_prob(action)
        dist_entropy = dist.entropy()
        critic_w, critic_b, critic_emb = self.critic_hyper(meta_v, pos_emb=pos_emb, scale=self.scale)
        critic_base_out = self.critic_base(torch.cat([state, actor_emb], dim=1))
        state_values = torch.bmm(critic_w, critic_base_out.unsqueeze(2)) + critic_b
        state_values = torch.squeeze(state_values, dim=2)
        return action_logprobs, state_values, dist_entropy
...
class HyperAC2LayerNoAug(nn.Module):
    ...
    def __init__(self, meta_v_dim, state_dim, action_dim, hyper_hidden_dim=128, dynamic_hidden_dim=128):
        ...
        self.actor_base = nn.Sequential(nn.Linear(state_dim, dynamic_hidden_dim),
                                        nn.ReLU(),
                                        nn.Linear(dynamic_hidden_dim, dynamic_hidden_dim),
                                        nn.ReLU()
                                        )
        self.actor_base.apply(init_weights_kaiming_uniform)
        self.actor_hyper = HyperNetwork2Layer(meta_v_dim, action_dim, hyper_hidden_dim, dynamic_hidden_dim)
        self.critic_base = nn.Sequential(nn.Linear(state_dim, dynamic_hidden_dim),
                                         nn.ReLU(),
                                         nn.Linear(dynamic_hidden_dim, dynamic_hidden_dim),
                                         nn.ReLU()
                                         )
    ...
    def act(self, meta_v, state, ret_prob=False, pos_emb=None):
        actor_w, actor_b = self.actor_hyper(meta_v)
        actor_base_out = self.actor_base(state)
        action_probs = torch.bmm(actor_w, actor_base_out.unsqueeze(2)) + actor_b
        action_probs = torch.squeeze(action_probs, dim=2)
        dist = Categorical(logits=action_probs)
        action = dist.sample()
    ...
    def evaluate(self, meta_v, state, action, pos_emb=None):
        ...
        dist_entropy = dist.entropy()
        critic_w, critic_b = self.critic_hyper(meta_v)
        critic_base_out = self.critic_base(state)
        state_values = torch.bmm(critic_w, critic_base_out.unsqueeze(2)) + critic_b
        state_values = torch.squeeze(state_values, dim=2)
        return action_logprobs, state_values, dist_entropy
...
class SimpleHyperPPONoHeadLastLayer:
    def __init__(self, meta_v_dim, meta_v_emb, meta_v_emb_dim, state_dim, action_dim, lr_a, lr_c, gamma, K_epochs, eps_clip, device, cuda,
        ...
        if fix_critic:
            ...
            if optimizer == 'adam':
                self.optimizer = torch.optim.Adam([
                    {'params': list(self.policy.actor_base.parameters()) + list(self.policy.actor_hyper.parameters()), 'lr': lr_a},
                    {'params': list(self.policy.critic_base.parameters()) + list(self.policy.critic_hyper.parameters()), 'lr': lr_c}
                ], weight_decay=w_decay)
            elif optimizer == 'adamw':
                self.optimizer = torch.optim.AdamW([
                    {'params': self.policy.actor.parameters(), 'lr': lr_a},
