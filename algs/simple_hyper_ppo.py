...
class HyperAC(nn.Module):
    ...
    def evaluate(self, meta_v, state, action, actor_get_layer_out=False, actor_layer=None, critic_get_layer_out=False,
                 critic_layer=None, pos_emb=None):
...
class HyperACEmbedInput(nn.Module):
    ...
    def __init__(self, meta_v_dim, meta_v_emb_dim, state_dim, action_dim,
                 hidden_dim=256, clip_emb=False):
    ...
    def act(self, meta_v, state, ret_prob=False, pos_emb=None):
        action_probs, _ = self.actor(meta_v, state, pos_emb=pos_emb)
        dist = Categorical(logits=action_probs)
        action = dist.sample()
        action_logprob = dist.log_prob(action)
    ...
    def evaluate(self, meta_v, state, action, actor_get_layer_out=False, actor_layer=None, critic_get_layer_out=False,
                 critic_layer=None, pos_emb=None):
        action_probs, actor_layer_out = self.actor(meta_v, state, get_layer_out=actor_get_layer_out, layer=actor_layer, pos_emb=pos_emb)
        dist = Categorical(logits=action_probs)
        action_logprobs = dist.log_prob(action)
        dist_entropy = dist.entropy()
        state_values, critic_layer_out = self.critic(meta_v, state, get_layer_out=critic_get_layer_out, layer=critic_layer, pos_emb=pos_emb)
...
class SimpleHyperPPO:
    def __init__(self, meta_v_dim, meta_v_emb, meta_v_emb_dim, state_dim, action_dim, lr_a, lr_c, gamma, K_epochs, eps_clip, device, cuda,
                 min_meta_v, max_meta_v, pos_emb, pos_emb_dim, hidden_dim=256, hyper_actor=True, hyper_critic=True, reg_actor=False,
