...
def render(tensor, orders):
    ...
    def init():
        ...
        return scatter_blue, scatter_red

    def update(frame):
        current_blue_points = tensor[frame]
        ...
        return scatter_blue, scatter_red, *annotations

    ani = FuncAnimation(fig, update, frames=num_frames, init_func=init, blit=True, interval=500)
    ani.save("rollout.gif", writer='imagemagick')
    plt.show()


file = np.load('render/rollout_100.npz')
agents = file['agents']
orders = file['orders']

render(agents, orders)